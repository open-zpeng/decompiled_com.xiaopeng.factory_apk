...
public class TextSimilarity {
    ...
    private static boolean isValidChar(char charValue) {
        return (charValue >= 19968 && charValue <= 40959) || (charValue >= 'a' && charValue <= 'z') || ((charValue >= 'A' && charValue <= 'Z') || (charValue >= '0' && charValue <= '9'));
    }
    ...
    private static int[][] generateMatrix(String strA, String strB) {
        int m = strA.length();
        int n = strB.length();
        int[][] matrix = (int[][]) Array.newInstance(int.class, m + 1, n + 1);
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
