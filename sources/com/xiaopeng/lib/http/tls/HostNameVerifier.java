...
public final class HostNameVerifier implements HostnameVerifier {
    ...
    private boolean verifyHostname(String hostname, X509Certificate certificate) {
        ...
        if (!hasDns) {
            ...
            String cn2 = new DistinguishedNameParser(principal).findMostSpecific("cn");
            if (cn2 != null) {
                return verifyHostname(hostname2, cn2);
            }
            return false;
        }
        return false;
    ...
    public boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(".") || hostname.endsWith(StrPool.DOUBLE_DOT) || pattern == null || pattern.length() == 0 || pattern.startsWith(".") || pattern.endsWith(StrPool.DOUBLE_DOT)) {
            return false;
        }
