...
public class BizRequestBuilder {
        ...
        for (int index = 0; index < keyList.size(); index++) {
            ...
            if (paramKeys.contains(key) || key.startsWith(BizConstants.HEADER_PREFIX)) {
                ...
                if (!TextUtils.isEmpty(value)) {
                    if (builder.length() > 0) {
                        builder.append(CharPool.AMP);
                    }
                    builder.append(key + Constant.EQUALS_STRING + value);
                }
            }
        }
    ...
    private String generateXpClient() {
        ...
        StringBuilder builder = new StringBuilder();
        builder.append("di=" + getHardwareId() + Constant.SEMICOLON_STRING);
        builder.append("pn=" + GlobalConfig.getApplicationName() + Constant.SEMICOLON_STRING);
        builder.append("ve=" + BuildInfoUtils.getSystemVersion() + Constant.SEMICOLON_STRING);
        builder.append("br=Xiaopeng;");
        builder.append("mo=" + model2 + Constant.SEMICOLON_STRING);
        builder.append("st=1;");
        builder.append("sv=" + BuildInfoUtils.getSystemVersion() + Constant.SEMICOLON_STRING);
        builder.append("nt=" + getNetworkType() + Constant.SEMICOLON_STRING);
        builder.append("t=" + System.currentTimeMillis() + Constant.SEMICOLON_STRING);
    ...
    private String generateSignature() {
        ...
        try {
            ...
            String signedOrigin = method + CharPool.AMP + parameter + CharPool.AMP + md5;
            LogUtils.d(TAG, "generateSignature origin:" + signedOrigin);
            byte[] sha256Encode = hmacSHA256.doFinal((method + CharPool.AMP + parameter + CharPool.AMP + md5).getBytes("UTF-8"));
