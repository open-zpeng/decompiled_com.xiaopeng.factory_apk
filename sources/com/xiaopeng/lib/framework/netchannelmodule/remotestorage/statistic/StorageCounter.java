    ...
    private void commitData() {
        ...
        if (requestCount >= MAX_VALUE_THRESHOLD || trafficSize >= MAX_VALUE_THRESHOLD || this.mLastDate.get() < currentDate) {
            ...
            if (!international) {
                ...
                dataLogService.sendStatData(dataLogService.buildMoleEvent().setEvent(EVENT_NAME).setPageId(GlobalConfig.MOLE_PAGE_ID).setButtonId(MOLE_TRAFFIC_OSS_BUTTON_ID).setProperty(STAT_KEY_PACKAGE_NAME, GlobalConfig.getApplicationSimpleName()).setProperty(STAT_KEY_REQUEST, Long.valueOf(requestCount)).setProperty("fail", Long.valueOf(fail)).setProperty(STAT_KEY_SUCCEED, Long.valueOf(success)).setProperty(STAT_KEY_REJECT, Long.valueOf(reject)).setProperty("size", Long.valueOf(trafficSize)).setProperty(STAT_KEY_ERROR_CODES, errorCodes).setProperty("date", Long.valueOf(lastDate)).build());
            }
        }
    }
    ...
    private String getErrorCodesInString() {
        ...
        synchronized (this) {
            for (String key : this.mErrorCodes.keySet()) {
                builder.append(key + ":" + this.mErrorCodes.get(key) + Constant.SEMICOLON_STRING);
            }
        }
