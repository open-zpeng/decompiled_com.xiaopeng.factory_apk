        ...
        try {
            ...
            if (STATUS_OK == result.getStatusCode()) {
                ...
                if (!international) {
                    ...
                    dataLogService.sendStatData(dataLogService.buildStat().setEventName(GlobalConfig.EVENT_NAME_SUCCESS).setProperty("pack", GlobalConfig.getApplicationSimpleName()).setProperty("method", RequestParameters.SUBRESOURCE_APPEND).setProperty("localSize", Integer.valueOf(this.mUploadContent.length)).setProperty("uploadPath", this.mRemoteObjectKey).setProperty("requestId", result.getRequestId()).build());
                }
                doSuccess();
            ...
            if (!international2) {
                ...
                dataLogService2.sendStatData(dataLogService2.buildStat().setEventName(GlobalConfig.EVENT_NAME_FAIL).setProperty("pack", GlobalConfig.getApplicationSimpleName()).setProperty("method", RequestParameters.SUBRESOURCE_APPEND).setProperty("localSize", Integer.valueOf(this.mUploadContent.length)).setProperty("uploadPath", this.mRemoteObjectKey).setProperty("failReason", errorCode).setProperty(RequestParameters.UPLOAD_ID, (String) null).build());
            }
            ...
            if (!StorageCounter.isInternationVersion()) {
                ...
                dataLogService3.sendStatData(dataLogService3.buildStat().setEventName(GlobalConfig.EVENT_NAME_FAIL).setProperty("pack", GlobalConfig.getApplicationSimpleName()).setProperty("method", RequestParameters.SUBRESOURCE_APPEND).setProperty("localPath", this.mLocalFilePath).setProperty("localSize", Integer.valueOf(this.mUploadContent.length)).setProperty("uploadPath", this.mRemoteObjectKey).setProperty("failReason", e.toString()).setProperty(RequestParameters.UPLOAD_ID, (String) null).build());
            }
