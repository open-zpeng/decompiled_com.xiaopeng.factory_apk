...
public abstract class BaseOssTask implements Runnable {
    ...
    private static final byte[] KEY = {123, 54, 90, 56, 23, 74, 89, 71, 55, TestResultItem.RESULT_NOTEST, 65, 40, 100};
    private static final byte[] ENCRYPT_PASSWORD = {3, 95, 59, 87, 103, 47, 55, WlanPresenter.WL_RMMOD_INSMOD, 88, 61, 50};
    ...
    public synchronized OSS createOssClient(@NonNull String accessKeyId, @NonNull String accessKeySecret, @NonNull String securityToken) {
        ...
        if (sOssClient == null) {
                ...
                try {
                    try {
                        if (this.mCertified) {
                            ...
                            sOssClient = new OSSClient(this.mApplication, this.mBucket.getPrivateUrl(), ossCredentialProvider, configuration);
                        } else {
            ...
            sOssClient.updateCredentialProvider(ossCredentialProvider);
        }
        return sOssClient;
    }
    ...
    private OkHttpClient getCustomOkHttpClient() throws NoSuchAlgorithmException, CertificateException, KeyStoreException, IOException, UnrecoverableKeyException, KeyManagementException {
        ...
        CertificateFactory cf = CertificateFactory.getInstance(KeyUtil.CERT_TYPE_X509);
