...
public class SystemPropertyUtil {
    ...
    public static void setAccountUid(long uid) {
        SystemProperties.set(SYSTEM_PROPERTY_ACCOUNT_UID, uid + "");
    }
    ...
    public static void setVehicleId(int vehicleId) {
        SystemProperties.set(SYSTEM_PROPERTY_VEHICLE_ID, vehicleId + "");
    }
    ...
    public static String getVIN() {
        String result = SystemProperties.get(SYSTEM_PROPERTY_PERSIST_VIN, "");
        if (TextUtils.isEmpty(result)) {
            return SystemProperties.get(SYSTEM_PROPERTY_VIN, "");
        }
        return result;
    }
    ...
    public static void setAiosWakeUpEnable(boolean enable) {
        SystemProperties.set(SYSTEM_PROPERTY_AIOS_WAKEUP, enable ? "1" : "0");
    }
    ...
    public static void setInCall(boolean enable) {
        SystemProperties.set(SYSTEM_PROPERTY_CALLING, enable ? "1" : "0");
    }

    public static void setHardwareId(String hardwareId) {
        if (Build.VERSION.SDK_INT >= 26) {
            SystemProperties.set("persist.sys.mcu.hardwareId", hardwareId);
        } else {
            SystemProperties.set(SYSTEM_PROPERTY_HARDWARE_ID, hardwareId);
        }
    }
    ...
    public static String getHwVersion() {
        ...
        if (Build.VERSION.SDK_INT > 28) {
            info = SystemProperties.get(SYSTEM_PROPERTY_HW_VERSION_Q);
        } else {
            info = SystemProperties.get(SYSTEM_PROPERTY_HW_VERSION);
        }
        return TextUtils.isEmpty(info) ? Support.Properties.PROPERTIES_DEFAULT_STRING : info;
    }
    ...
    public static boolean isRepairMode() {
        return "on".equals(getRepairModeStr());
    }
    ...
    public static String getSCU() {
        return SystemProperties.get(SYSTEM_PROPERTY_SCU, "");
    }
