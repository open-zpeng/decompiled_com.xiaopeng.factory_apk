...
public class KeyframeSet {
    ...
    public Object getValue(float fraction) {
        ...
        if (i == 2) {
            ...
            float intervalFraction2 = (fraction - prevFraction2) / (this.mLastKeyframe.getFraction() - prevFraction2);
            return this.mEvaluator.evaluate(intervalFraction2, prevKeyframe.getValue(), this.mLastKeyframe.getValue());
        } else {
            ...
            for (int i2 = 1; i2 < this.mNumKeyframes; i2++) {
                ...
                if (fraction < nextKeyframe2.getFraction()) {
                    Interpolator interpolator4 = nextKeyframe2.getInterpolator();
                    if (interpolator4 != null) {
                        fraction = interpolator4.getInterpolation(fraction);
                    }
                    float prevFraction3 = prevKeyframe2.getFraction();
                    float intervalFraction3 = (fraction - prevFraction3) / (nextKeyframe2.getFraction() - prevFraction3);
                    return this.mEvaluator.evaluate(intervalFraction3, prevKeyframe2.getValue(), nextKeyframe2.getValue());
                }
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.mNumKeyframes; i++) {
            returnVal = returnVal + this.mKeyframes.get(i).getValue() + Constant.SPACE_2_STRING;
        }
        return returnVal;
    }
}
