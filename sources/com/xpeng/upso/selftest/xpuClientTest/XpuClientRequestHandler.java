...
public class XpuClientRequestHandler implements Handler.Callback {
    ...
    private static final String TAG = "Upso-XPU_CH";
    ...
    private XpuClientPreset xpuClientPreset = new XpuClientPreset();
    private Handler handler = new Handler(Looper.getMainLooper(), this);

    public XpuClientRequestHandler(Context context, Socket socket, XpengCarModel carModel, XpengCarChipModel chipModel, int op) {
    ...
    private String getDecryptResult(byte[] buffer) {
        ...
        try {
            ...
            try {
                String alais = this.xpuClientPreset.genAlias(teeEncrpytedContent.getKeyIndex().intValue(), this.carModel, this.chipModel);
                String resultString = this.xpuClientPreset.decryptAesCbcPkcs7(alais, teeEncrpytedContent);
                return resultString;
            } catch (Exception e3) {
                e = e3;
                e.printStackTrace();
    ...
    private boolean handleInfo(PSOProtocol.RequestResponse request) {
        ...
        identityInfoBuilder.setRomInfo("XPU_V0.899999");
        if (this.op == 1) {
            ...
            boolean isPreseted = this.xpuClientPreset.isPreseted(this.carModel, this.chipModel);
    ...
    private boolean handleSecretPreset(PSOProtocol.RequestResponse request) {
        ...
        if (secrets != null) {
            ...
            for (PSOProtocol.Secret secret : secrets) {
                try {
                    String alias = this.xpuClientPreset.genAlias(secret.getIndex(), this.carModel, this.chipModel);
                    this.xpuClientPreset.delAesKey(alias);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            for (PSOProtocol.Secret secret2 : secrets) {
                try {
                    if (secret2.getType() == PSOProtocol.Secret.SecretType.KEY_TYPE_AES && (secret2.getName() == null || secret2.getName().length() < 1)) {
                        String alias2 = this.xpuClientPreset.genAlias(secret2.getIndex(), this.carModel, this.chipModel);
                        this.xpuClientPreset.presetAesKeyByData(alias2, HexUtils.hexStringToByte(secret2.getSecret()));
                    }
                    index++;
    ...
    */
    private boolean handleSecretEncrypt(com.xpeng.upso.proxy.PSOProtocol.RequestResponse r22) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.xpeng.upso.selftest.xpuClientTest.XpuClientRequestHandler.handleSecretEncrypt(com.xpeng.upso.proxy.PSOProtocol$RequestResponse):boolean");
    }
    ...
    private String getDeviceType() {
        return XpengPsoClientType.XPU.toString();
    }
