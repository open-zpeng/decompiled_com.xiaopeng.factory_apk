...
public class HostKey {
    ...
    public String getType() {
        ...
        return Constant.UNKNOWN_STRING;
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public static int name2type(String name) {
        int i = 0;
        while (true) {
            byte[][] bArr = names;
            if (i < bArr.length) {
                if (!Util.byte2str(bArr[i]).equals(name)) {
                    i++;
                } else {
                    return i + 1;
                }
    ...
    private boolean isIncluded(String _host) {
        ...
        while (i < hostslen) {
            ...
            } else if (hostlen == j - i && hosts.regionMatches(true, i, _host, 0, hostlen)) {
                return true;
            } else {
                i = j + 1;
            }
